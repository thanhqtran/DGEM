# AK70_prog_pensions_procs.jl
#
# input into "AK70_prog_pension_main.jl"
...
function pension(x,meany,pmin)    # pension schedule: see Huggett and Parra (2010, JPE)
	bendpoint1 = 0.20*meany
  	bendpoint2 = 1.24*meany

    if x<bendpoint1
        y = pmin+0.9*x
    elseif (x>=bendpoint1) & (x<=bendpoint2)
        y = pmin + 0.9*bendpoint1 + 0.32*(x-bendpoint1)
    else
        y = pmin+ 0.9*bendpoint1 + 0.32*(bendpoint2-bendpoint1) + 0.15*(x-bendpoint2)
    end
...
function get_gini(x,m)
	...
	if abs(sum(m)-1.0)>1e-12
		println("")
		println("*********************************************")
		println("total mass in get_gini() is not eqqal to one!")
...
function uc1(a1,iperm,iy,iage,cwopt,lopt,cropt,e1)
	ia1 = sum(a.<=a1)
    if iage<=nw
        if e1<=0
            c1 = cwopt[iperm,iy,ia1,1,iage]
            labor = lopt[iperm,iy,ia1,1,iage]
        elseif e1>=ymax
            c1 = cwopt[iperm,iy,ia1,nce,iage]
            labor =  lopt[iperm,iy,ia1,nce,iage]
        else
            ice1=sum(e.<e1)+1
            ice1=min(ice1,nce)
            n0=(e1-e[ice1-1])/(e[ice1]-e[ice1-1])
			# linear interpolation
            c1 = (1-n0)*cwopt[iperm,iy,ia1,ice1-1,iage] + n0*cwopt[iperm,iy,ia1,ice1,iage]
            labor = (1-n0)*lopt[iperm,iy,ia1,ice1-1,iage] + n0*lopt[iperm,iy,ia1,ice1,iage]
        end
    else
        labor = 0
        ice1 = sum(e.<e1)+1
        c1 = cropt[ia1,ice1,iage-nw]
    end
    return uc(c1,labor)
end
