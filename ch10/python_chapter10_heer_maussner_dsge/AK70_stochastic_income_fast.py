...
def value2(x):
    ...
    if iage==nw-1:
        y = u(c,labor)+ygrowth**(gamma*(1-eta))*sp1[iage]*beta1*vw_polate(k1)
    else:
        y = u(c,labor)
        for iy1 in range(ny): 
            # interpolation of vw at age iage in the next period of life
            vw_polate0 = vw_polate[iy1]
            y = y + py[iy,iy1]*ygrowth**(gamma*(1-eta))*sp1[iage]*beta1*vw_polate0(x)
    return y
...
while q<nq-1 and crit>tol:
    ...
    for iage in range(nw-1,-1,-1):
        ...
        for iperm in range(nperm):
            print(iperm)
            # define array of objects (interpolation for different types iy)            
            vw_polate =[]
            if iage==nw-1:          
                vw_polate = interpolate.interp1d(a,vr[:,0], kind=kind_tol) # interpolation of vr at age iage+1
            else:
                for iy1 in range(ny):
                    vw_polate0 = interpolate.interp1d(a,vw[:,iy1,iperm,iage+1], kind=kind_tol)
                    vw_polate.append(vw_polate0)
